...
function normalizePlan(value: unknown, fallback: Plan): Plan {
  if (typeof value !== 'string') return fallback
  const candidates = COPY.en.plans as Record<string, string>
  return value in candidates ? value as Plan : fallback
}

export function ScannerClient({ plan, userName }: { plan: string; userName: string }) {
  const initialPlan = normalizePlan(plan, 'free')
  const [effectivePlan, setEffectivePlan] = useState<Plan>(initialPlan)
  const typedPlan = effectivePlan
  ...
  useEffect(() => {
    fetch('/api/food-scan')
      .then(r => r.json())
      .then(d => {
        setEffectivePlan(current => normalizePlan(d.plan, current))
        setUsed(d.used ?? 0)
        setLimit(d.limit ?? 1)
...
function MacroBar({ label, value, color, max }: { label: string; value: number; color: string; max: number }) {
  ...
  return <div className="fs-macro-bar-row"><div className="fs-macro-bar-label">{label}</div><div className="fs-macro-bar-track"><div className="fs-macro-bar-fill" style={{ width: `${width}%`, background: color }} /></div><div className="fs-macro-bar-val">{value}g</div></div>
}

function MacroChip({ label, value, color }: { label: string; value: number; color: string }) {
  return <div className="fs-macro-chip"><div className="fs-macro-chip-label" style={{ color }}>{label}</div><div className="fs-macro-chip-val">{value}g</div></div>
}
