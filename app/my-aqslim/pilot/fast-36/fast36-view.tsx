...
const STATUS_LABELS: Record<Fast36Status, { es: string; en: string }> = {
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.content}>
        ...
        <h1>{status === 'active' ? (es ? 'Vas muy bien.' : 'You are doing great.') : (es ? 'Tu programa Fast 36.' : 'Your Fast 36 program.')}</h1>
        ...
        <section className={styles.timerCard}>
          <p>{status === 'pending' ? (es ? 'COMIENZA EN' : 'STARTS IN') : (es ? 'TIEMPO RESTANTE' : 'TIME REMAINING')}</p>
          ...
          <div className={styles.timerMeta}><span>{Math.round(progress)}% {es ? 'completado' : 'completed'}</span><span>36 {es ? 'horas' : 'hours'}</span></div>
        </section>

        <section className={styles.schedule}>
          <div><small>{es ? 'ÚLTIMA COMIDA' : 'LAST MEAL'}</small><strong>{formatDate(current.startAt, language)}</strong></div>
          <div><small>{es ? 'PRÓXIMA COMIDA' : 'NEXT MEAL'}</small><strong>{formatDate(current.plannedEndAt, language)}</strong></div>
        </section>
            ...
            {sessions.map(session => {
              ...
              return (
                <article key={session.id} className={sessionStatus === 'active' ? styles.activeWeek : ''}>
                  ...
                  <small>{formatDate(session.startAt, language)}</small>
                </article>
              )
            })}
          </div>
